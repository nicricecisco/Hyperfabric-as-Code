...
def get_fabric_new_objects(new_object_data_obj):
    """
     Gets a list of newObjects for a fabric

     Args:
         new_object_data_obj (dict): A dictionary containing fabric ID. Expected keys:
            - "fabric_id" (str): The ID or name of the fabric.

    Returns:
        dict: JSON response, or None on error.
    """
    params = {key: new_object_data_obj["new_object"][key] for key in ["candidate", "includeMetadata"] if key in new_object_data_obj.get("new_objects", {})}
    
    fabricId = new_object_data_obj["fabric_id"]
    response = _make_get_request(headers, f"{BASE_URL}/fabrics/{fabricId}/newObjects", params=params)
    return response

def add_fabric_new_objects(new_object_data_obj):
    """
    Creates or updates one or more newObject objects for a fabric fabric object.

    Args:
        new_object_data_obj (dict): A dictionary containing fabric ID and newObjects data. Expected keys:
            - "fabric_id" (str): The ID or name of the fabric.
            - "new_object" (dict): A dictionary representing a single newObject object to add.
              
              Note: The function wraps this single newObject in a list for the API call.

    Returns:
        dict: JSON response, or None on error.
    """
    payload = {"newObjects": [new_object_data_obj["new_object"]]}
    
    fabricId = new_object_data_obj["fabric_id"]
    response = _make_post_request(headers,f"{BASE_URL}/fabrics/{fabricId}/newObjects",payload=payload)
    return response

def get_fabric_new_object(new_object_data_obj):
    """
    Gets information for a single newObject.

    Args:
        new_object_data_obj (dict): A dictionary containing fabric ID and newObject data. Expected keys:
            - "fabric_id" (str): The ID or name of the fabric.
            - "new_object" (dict): A dictionary containing the newObject's name.
                Expected sub-keys:
                - "name" (str): The ID or name of the newObject.

    Returns:
        int: JSON response on success or None on Fail
    """
    params = {key: new_object_data_obj["new_object"][key] for key in ["candidate", "includeMetadata"] if key in new_object_data_obj["new_object"]}
    
    fabricId = new_object_data_obj["fabric_id"]
    newObjectId = new_object_data_obj["new_object"]["name"]
    response = _make_get_request(headers, f"{BASE_URL}/fabrics/{fabricId}/newObjects/{newObjectId}", params=params)
    return response

def update_fabric_new_object(new_object_data_obj):
    """
    Updates a specific newObject.

    Args:
        new_object_data_obj (dict): A dictionary containing fabric ID, newObject ID, and updated newObject properties. Expected keys:
            - "fabric_id" (str): The ID or name of the fabric.
            - "new_object" (dict): A dictionary containing the updated newObject properties. Must include "name" for newObject ID.
    ...
    fabricId = new_object_data_obj["fabric_id"]
    newObjectId = new_object_data_obj["new_object"]["name"]
    payload = new_object_data_obj["new_object"]
    response = _make_put_request(headers, f"{BASE_URL}/fabrics/{fabricId}/newObjects/{newObjectId}", payload=payload)
    return response

def delete_fabric_new_object(new_object_data_obj):
    """
    Deletes a newObject.

    Args:
        new_object_data_obj (dict): A dictionary containing fabric ID, and newObject ID. Expected keys:
            - "fabric_id" (str): The ID or name of the fabric.
            - "new_object" (dict): A dictionary containing the name of the newObject to delete.
    ...
    fabricId = new_object_data_obj["fabric_id"]
    newObjectId = new_object_data_obj["new_object"]["name"]
    response = _make_delete_request(headers, f"{BASE_URL}/fabrics/{fabricId}/newObjects/{newObjectId}")
    return response
